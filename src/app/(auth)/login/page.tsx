...
function LoginForm() {
  ...
  /** Roles that should go to /profile instead of admin dashboard */
  const candidateRoles = ["CANDIDATE"];

  /** After login, decide where to redirect based on role */
  const getRedirectUrl = async (): Promise<string> => {
    // Fetch the fresh session to check tenant/role
    const session = await getSession();

    const roleName = session?.role?.name ?? "";
    const isCandidate = candidateRoles.includes(roleName);

    // If there's a callbackUrl from query params, validate it
    if (callbackUrl) {
      // Don't let candidate users follow callbackUrl to admin pages
      if (isCandidate && callbackUrl.startsWith("/admin")) {
        return "/profile";
      }
      return callbackUrl;
    }

    // Candidate users → profile area
    if (isCandidate || !session?.tenant?.id) return "/profile";
  ...
  const handleGoogleLogin = () => {
    const defaultUrl = callbackUrl ?? "/";
    signIn("google", { callbackUrl: defaultUrl });
  };
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="mb-8 flex flex-col items-center">
        ...
        <h1 className="text-2xl font-bold">เข้าสู่ระบบ U-Exam</h1>
